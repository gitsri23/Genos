...
// --- Home Screen (Stack Flow Animation) ---
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  // PageController for the 3D Swipe Effect
    ...
    return Scaffold(
      ...
      body: _isLoading 
          ...
          : _albums.isEmpty 
              ...
              : Center(
                  child: SizedBox(
                    height: 400, // Height of the swipe cards
                      ...
                      itemBuilder: (context, index) {
                        return AnimatedBuilder(
                          ...
                          builder: (context, child) {
                            // Zoom In/Out Logic based on scroll position
                            double value = 1.0;
                          ...
                          child: GestureDetector(
                            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => FolderVideosScreen(album: _albums[index]))),
                            child: PremiumGlass(
                              borderRadius: 30,
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(_albums[index].name, style: const TextStyle(color: Colors.white, fontSize: 28, fontWeight: FontWeight.bold), textAlign: TextAlign.center, maxLines: 2),
                                  const SizedBox(height: 10),
                                  FutureBuilder<int>(
                                    future: _albums[index].assetCountAsync,
                                    builder: (_, snapshot) => Text('${snapshot.data ?? 0} Videos', style: const TextStyle(color: Colors.white54, fontSize: 18)),
                                  ),
                                ],
                              ),
                            ),
...
// --- Folder Videos Screen (With Thumbnails) ---
...
class _FolderVideosScreenState extends State<FolderVideosScreen> {
  ...
  Future<void> _fetchVideos() async {
    ...
    final List<AssetEntity> videos = await widget.album.getAssetListRange(start: 0, end: count);
  ...
  void _playVideo(AssetEntity video) async {
    ...
    if (file != null && mounted) {
      Navigator.push(context, MaterialPageRoute(builder: (_) => ProVideoPlayerScreen(videoPath: file.path)));
    }
  }
                    ...
                    child: SizedBox(
                      ...
                      child: FutureBuilder<Uint8List?>(
                        ...
                        builder: (_, snapshot) {
                          ...
                          return Container(color: Colors.white10, child: const Icon(Icons.video_file, color: Colors.white30));
                        },
                      ),
                    ),
                  ),
                  title: Text(video.title ?? 'Unknown Video', style: const TextStyle(color: Colors.white, fontSize: 14), maxLines: 2, overflow: TextOverflow.ellipsis),
                  subtitle: Text('${(video.duration / 60).floor()}:${(video.duration % 60).toString().padLeft(2, '0')}', style: const TextStyle(color: Colors.white54, fontSize: 12)),
